...
默认使用内置月球入口流程，不依赖快捷传送面板或 `/tp` 命令：调度器通过稳定的以太之光 ID `175` 调用游戏传送服务前往“最佳兔威洞”，通过 vnavmesh 移动到 `(21.9, 13.2, -1.4)`，查找并与“架行威”交互，自动推进对话和确认。只有严格确认已经进入所选 ICE 区域后才会切换职业并启动 ICE。
...
5. 观察状态依次经过“暂停 MissFisher / 传送 / 前往架行威 / 处理对话 / 切换职业 / 启动 ICE”。

入口对话默认选择第 `0` 项。如果 NPC 菜单中的目标入口不是第一项，在配置中调整“入口对话选项序号（从 0 开始）”。
